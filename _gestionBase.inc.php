...
function connect()
{
   try
   {
      ...
      $login="root";
      $mdp="root";
      $connect= new PDO("mysql:host=$hote;dbname=festival", $login, $mdp);
   }
...
function obtenirReqEtablissementsAyantChambresAttribuées()
{
   $req="select distinct idEtablissement, nom, nombreChambresOffertes from Etablissement, 
         Attribution order by idEtablissement";
   return $req;
}
...
{
   $req="select idE, nom, nomPays from Equipe where hebergement='O' order by idE";
   return $req;
}
...
{
   $req="select distinct idE, nom, nomPays from Equipe, Attribution where 
